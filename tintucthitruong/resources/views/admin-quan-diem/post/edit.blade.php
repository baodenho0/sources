    ...
    <form id="myform" class="form-horizontal" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <div class="col-sm-5">
                ...
                <textarea class="form-control" rows="5" style="width: 100%" id="description" name="description">{{$post->description?? ""}}</textarea>
                {{-- <label for="status">Trạng thái:</label> --}}
            </div>
            <div class="col-sm-7">
                <label for="image">Hình ảnh</label>
                <input type="file" class="dropify" data-default-file="{{asset($post->image_path)}}" name="image" id="image">
                <input type="hidden" id="image_hidden" name="image_hidden" value="{{$post->image_path!=null?$post->image_path:''}}">
            </div>
        </div>
        <div class="form-group">
            <div class="col-sm-12">
                <label for="body-summernote">Nội Dung:</label>
                <textarea id="body-summernote" name="body">{{$post->body?? ""}}</textarea>
            </div>
        </div>
