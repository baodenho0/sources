...
@section('content')
    <div class="box box-block bg-white">
        <div class="mb-1">
            <a href="{{ route('portfolios.create') }}" class="btn btn-success">Thêm mới bộ sưu tập</a>
        </div>
        <table class="table table-striped table-bordered" id="users">
            ...
            @foreach($portfolios as $portfolio)
                <tr>
                    ...
                    <td><img width="200px" src="{{ asset($portfolio->image_path) }}" alt=""></td>
                    <td>{{  $portfolio->created_at->format('d-m-Y H:i') }}</td>
                    <td>
                        <a class="btn btn-primary btn-sm" href="{{ route('portfolios.show', $portfolio->id) }}"> <i
                                    class="ti-eye"></i> Xem</a>
                        <a class="btn btn-warning btn-sm" href="{{ route('portfolios.edit',$portfolio->id) }}"> <i
                                    class="ti-pencil"></i> Sửa</a>
                        <button type="button" data-toggle="modal" data-target="#delete"
                                data-href="{{ route('portfolios.delete',$portfolio->id) }}" class="btn btn-danger btn-sm">
